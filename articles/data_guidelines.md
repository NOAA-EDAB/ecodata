# SOE Data Structure Guidelines

The NEFSC State of the Ecosystem (SOE) report is a collection of
...
There are two different formats for structuring data: wide format and
...
While this is convenient for reading and human interpretation, many
analyses and graphing routines prefer the long format:
...
The long format makes it easier to generically subset the data to be
...
Time: Time stamp of the variable. Typically this is the year but could
be day of the week or month, etc. Var: Name of variable. Use a short
description such as “Piscivore Landings”. Value: The numerical value for
the data point. Units: Units for the variable. See standardized list
below. EPU: Ecological production unit or shelf-wide. See standardized
list below.

### Standarized lists

Here is the list of standardized unit notation. Note that this list may
not be comprehensive. However this should give you an idea of how to
...
Please use the abbreviations below to indicate to which ecological
production unit (EPU) your indicator belongs. This will aid in
separating the data between the Mid-Atlantic and New England.
...
`data.table` as well as a `tidyverse` solution. Run this code in R and
follow the example. Note the are `R` examples.
