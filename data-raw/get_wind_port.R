...
get_wind_port <- function(save_clean = F){
  ...
  wind_port<-read.csv(file.path(raw.dir, wind_port_csv)) %>%
    ...
    dplyr::select(State, City, perc_MIN, perc_MAX, Demographics, Gentrification, MaxVal) %>%
    tidyr::pivot_longer(cols = c(perc_MIN, perc_MAX, Demographics, Gentrification, MaxVal),
