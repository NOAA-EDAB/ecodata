...
ch_bay_temp_csv <- "GR_Sea_Water_Temp_Data - Charles Pellerin - NOAA Affiliate.csv"
get_ch_bay_temp <- function(save_clean = F) {
  ch_bay_temp <- read_csv(
    file.path(raw.dir, ch_bay_temp_csv),
    col_names = FALSE
  ) %>%
    janitor::row_to_names(1) %>%
    dplyr::rename(
      "Year" = "2024  Daily",
      "YearLTA" = "2010-2024 avg",
      "minLTA" = "2010-2024 min",
      "maxLTA" = "2010-2024 max"
    ) %>%
    tidyr::pivot_longer(
      c("Year", "YearLTA", "minLTA", "maxLTA"),
      names_to = "Var",
      values_to = "Value"
    ) %>%
    dplyr::mutate(Units = c("C"), Value = as.numeric(Value)) %>%
    dplyr::rename(Time = Date) %>%
    tidyr::separate(Time, c("Time", "Trash"), sep = " ") %>%
    dplyr::select(-Trash) %>%
    dplyr::mutate(Time = lubridate::mdy(Time), EPU = c("MAB")) %>%
  ...
  attr(
    ch_bay_temp,
    "tech-doc_url"
  ) <- "https://noaa-edab.github.io/tech-doc/chesapeake-bay-salinity-and-temperature.html"
