...
US_landings <- ecodata::comdat %>%
  dplyr::filter(Var == "Seafood Landings", Time >= 1986)
# #Total landings
total_landings <- ecodata::comdat %>%
  dplyr::filter(Var == "Landings", Time >= 1986)
...
landings_agg_pre2018 <- landings_agg %>%
  dplyr::filter(Time <= 2018, EPU == "GOM", Var == "Total")

landings_agg_post2018 <- landings_agg %>%
  dplyr::filter(Time > 2018, EPU == "GOM", Var == "Total")
landings_aggx <- landings_agg %>%
  dplyr::filter(EPU == "GOM", !Var == "Total")
...
landings_agg_pre2018 <- landings_agg %>%
  dplyr::filter(Time <= 2018, EPU == "GB", Var == "Total")

landings_agg_post2018 <- landings_agg %>%
  dplyr::filter(Time > 2018, EPU == "GB", Var == "Total")
landings_aggx <- landings_agg %>%
  dplyr::filter(EPU == "GB", !Var == "Total")
