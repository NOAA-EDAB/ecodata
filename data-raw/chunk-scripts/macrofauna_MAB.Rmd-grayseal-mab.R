ecodata::grayseal %>%
  ...
  ggplot2::geom_ribbon(
    aes(ymin = total5yLCI, ymax = total5yUCI, x = Time),
    fill = "blue",
    alpha = 0.2
  ) +
  ...
  ggplot2::scale_color_manual(
    name = element_blank(),
    values = c(
      '5yr rolling mean and CI' = 'blue',
      #                                      'NE Annual estimates' = 'black',
      'PBR' = 'red'
    )
  ) +
  theme(legend.position = c(0.25, 0.85), legend.text = element_text(size = 8)) +
  ecodata::theme_ts() +
  ecodata::theme_title()
