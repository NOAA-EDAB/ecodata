...
plot_stackbarcpts_single <- function(
  ...
  if (filt == TRUE) {
    ...
    dat2plot <-
      ...
      dplyr::mutate(
        var2bar = gsub(pattern = "_", replacement = " ", x = var2bar),
  ...
  p <-
    ggplot2::ggplot(dat2plot, aes(x = YEAR)) +
    ggplot2::geom_bar(
      data = dat2plot %>% filter(value > 0),
    ...
    {
      if (aggregate) geom_line(data = agg, aes(x = YEAR, y = Total), size = 1)
    } +
