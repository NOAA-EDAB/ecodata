prop <- ecodata::nefsc_survey_disaggregated %>%
  dplyr::filter(
    !stringr::str_detect(`Feeding guild`, "Other|other"),
    !is.na(Proportion)
  ) %>% #remove "other"
  tidyr::unite(., Var, c("Feeding guild", "Season"), sep = " ") %>%
  dplyr::filter(
    !stringr::str_detect(Var, "Apex|Benthos"),
    !is.na(Management)
  ) %>% #remove benthos and apex predators
  dplyr::group_by(EPU, Management, Var, Time) %>%
  dplyr::summarise(Proportion = sum(Proportion)) %>%
  dplyr::mutate(
    Var2 = paste(Management, Var),
  ...
  tidyr::complete(
    Time = full_seq(min(.$Time):max(.$Time), 1),
...
GOM_prop <-
  ...
  #  guides(color = FALSE) +
  ggplot2::geom_hline(
    aes(yintercept = hline, group = Var2, color = Management),
    size = 0.5,
    alpha = hline.alpha,
    linetype = hline.lty
  ) +
