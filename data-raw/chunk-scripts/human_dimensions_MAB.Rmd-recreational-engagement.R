com <- ecodata::engagement %>%
  dplyr::filter(Region == "MAB", Fishery == "Recreational") %>%
...
com2 <- com %>%
  ...
  ggrepel::geom_text_repel(
    aes(
      x = Eng, #geom_text_repel auto-jitters text around points
      y = Rel,
      label = Community,
      color = EJRating
    ),
  ...
  ggplot2::ggtitle(
    "Environmental Justice in Top Recreational Fishing Communities"
  ) +
