month <- seq(as.Date("2022-01-01"), as.Date("2022-12-31"), by = "1 month")
...
phyto_year_nano <- ecodata::phyto_size %>%
  ...
  dplyr::filter(year == 2022, !Value == "NA") %>%
  dplyr::mutate(Value = Value * 100)


phyto_year_micro <- ecodata::phyto_size %>%
  dplyr::filter(EPU == c("GB"), Var == c("WEEKLY_PSC_FMICRO_MEDIAN")) %>%
  mutate(Value = as.numeric(Value)) %>%
  tidyr::separate(., Time, into = c("Cat", "WEEK"), sep = "_") %>%
  dplyr::mutate(
    year = stringr::str_sub(WEEK, 1, 4),
...
phyto_year_micro2 <- ecodata::phyto_size %>%
  dplyr::filter(EPU == c("GOM"), Var == c("WEEKLY_PSC_FMICRO_MEDIAN")) %>%
  mutate(Value = as.numeric(Value)) %>%
  tidyr::separate(., Time, into = c("Cat", "WEEK"), sep = "_") %>%
  dplyr::mutate(
    year = stringr::str_sub(WEEK, 1, 4),
...
p2 <- ggplot2::ggplot() +
  ...
  #ggplot2::geom_point(data = chlor, aes(x = as.numeric(WEEK), y = Value)) +
  ggplot2::geom_line(
    aes(x = as.numeric(phyto_year_nano2$wk), y = phyto_year_nano2$Value),
    color = "#FC8D62",
    size = 1.5
  ) +
  ggplot2::geom_line(
    aes(x = as.numeric(phyto_year_micro2$wk), y = phyto_year_micro2$Value),
    color = "#66C2A5",
    size = 1.5
  ) +
  #ggplot2::facet_wrap(EPU~., ncol = 2)+
