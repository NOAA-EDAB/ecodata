...
apex <- ecodata::hms_landings %>%
  dplyr::filter(stringr::str_detect(Var, "Revenue"), Time < 2021) %>%
  separate(Var, c("Var", "trash"), sep = "_") %>%
  group_by(Time) %>%
  summarise(Value = sum(Value)) %>%
  mutate(Var = c("HMS Revenue"), Units = c("metric tons"), EPU = c("MAB"))
...
rev <- rbind(rev_agg, rev_total) %>%
  dplyr::filter(EPU == "MAB", Time > 1986)
#Plotting
ggplot2::ggplot(data = rev) +
  ...
  #lines
  ecodata::geom_gls(
    aes(x = Time, y = Total, group = Status),
    alpha = trend.alpha,
    size = trend.size
  ) +
  ...
  ggplot2::geom_hline(
    aes(yintercept = hline, color = Status),
    size = hline.size,
    alpha = hline.alpha,
    linetype = hline.lty
  ) +
