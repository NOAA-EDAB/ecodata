total <- ecodata::productivity_anomaly %>%
  ...
  dplyr::filter(EPU == "MA", Var == "rs_anom") %>%
  dplyr::group_by(Time) %>%
  dplyr::summarise(Total = sum(Value, na.rm = T), Count = n()) %>% # SG add a count of species
  dplyr::mutate(
    Totalold = ifelse(Total == 0, NA, Total),
...
p <-
  ggplot2::ggplot(prod, aes(x = Time)) +
  ggplot2::geom_bar(
    data = prod %>% filter(Value > 0),
  ...
  ggplot2::geom_line(data = total, aes(x = Time, y = Total), size = 1) +
