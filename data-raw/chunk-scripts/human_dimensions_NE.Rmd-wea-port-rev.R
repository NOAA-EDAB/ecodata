df.symbol <- ecodata::wind_port %>%
  filter(
    EPU == "NE",
    !Var %in% c("WEA_MAX", "TOT_MAX", "perc_MIN", "perc_MAX")
  ) %>%
  ...
  filter(!value == "NA") %>%
  dplyr::mutate(
    symbol = recode(Variable, EJ = -7, Gent = -3),
    Variable = recode(
      Variable,
...
df.all.perc <- ecodata::wind_port %>%
  ...
  pivot_longer(
    cols = c(perc_MIN, perc_dif, TOT_MAX),
    names_to = "Var",
    values_to = "Value"
  ) %>%
  ...
  dplyr::mutate(
    Var = recode(
      Var,
      "perc_MIN" = "WEA Revenue",
      "perc_dif" = "WEA Revenue Range",
      "TOT_MAX" = "Non-WEA Revenue"
    ),
    Var = factor(
      Var,
      levels = c("Non-WEA Revenue", "WEA Revenue Range", "WEA Revenue")
    )
  )
