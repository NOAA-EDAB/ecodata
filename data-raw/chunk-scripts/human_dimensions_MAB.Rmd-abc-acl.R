mean <- ecodata::abc.acl %>%
  ...
  dplyr::rename("abc.acl" = " ABC/ACL", "Catch" = " Catch") %>%
  dplyr::mutate(Value = Catch / abc.acl, Time = as.character(Time)) %>%
  filter(!Value == "NA", !Fishery == "Chub mackerel ") %>%
  dplyr::group_by(Time) %>%
...
ecodata::abc.acl %>%
  ...
  pivot_wider(names_from = Var, values_from = Value) %>%
  dplyr::rename("abc.acl" = " ABC/ACL", "Catch" = " Catch") %>%
  dplyr::mutate(Value = Catch / abc.acl, Time = as.numeric(Time)) %>%
  filter(!Value == "NA", !Fishery == "Chub mackerel ") %>%
  ggplot2::ggplot() +
  #geom_boxplot()+
