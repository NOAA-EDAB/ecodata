mass <- ecodata::mass_inshore_survey %>%
  ...
  dplyr::mutate(upper = Index + (2 * SE), lower = Index - (2 * SE)) %>%
  dplyr::group_by(Var) %>%
...
###Plot 1
p1 <- mass %>%
  dplyr::filter(!is.na(Var), stringr::str_detect(Var, "Piscivore")) %>%
  ...
  #Add time series
  ggplot2::geom_ribbon(
    aes(x = Time, ymin = pmax(lower, 0), ymax = upper),
    alpha = 0.5,
    fill = "grey"
  ) +
  ...
  ggplot2::geom_hline(
    aes(yintercept = hline, group = Var),
    size = hline.size,
    alpha = hline.alpha,
    linetype = hline.lty
  ) +
...
###Plot 2
p2 <- mass %>%
  dplyr::filter(!is.na(Var), stringr::str_detect(Var, "Benthivore")) %>%
  ggplot2::ggplot(aes(x = Time, y = Index)) +
...
###Plot 3
p3 <- mass %>%
  dplyr::filter(!is.na(Var), stringr::str_detect(Var, "Planktivore")) %>%
  ggplot2::ggplot(aes(x = Time, y = Index)) +
...
###Plot 4
p4 <- mass %>%
  dplyr::filter(!is.na(Var), stringr::str_detect(Var, "Benthos")) %>%
  ggplot2::ggplot(aes(x = Time, y = Index)) +
