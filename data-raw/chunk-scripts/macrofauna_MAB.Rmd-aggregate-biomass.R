agg <- ecodata::aggregate_biomass %>%
  ...
  tidyr::separate(
    Var,
    c("feeding.guild", "season", "Biomass", "Var1", "Null", "Area"),
    sep = " "
  ) %>%
  tidyr::unite("Var", feeding.guild:season, sep = " ") %>%
  dplyr::mutate(stat = recode(Var1, Index = "Mean", Standard = "SD")) %>%
  dplyr::select(-Biomass, -Var1, -Null, -Units) %>%
  # dplyr::filter(!Area == "-") %>%
  dplyr::group_by(Var, Time, EPU, Area) %>%
  tidyr::pivot_wider(
    id_cols = c(Var, Time, EPU, Area),
    names_from = stat,
    values_from = Value
  ) %>%
  dplyr::filter(!Mean == "NULL", !Time == 2020) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(Mean = as.numeric(Mean), SD = as.numeric(SD))


agg_bio <- agg %>%
  dplyr::filter(EPU == epu_abbr, Time >= 1968) %>%
...
p3 <- agg_bio %>%
  ...
  ggplot2::geom_ribbon(
    data = neamap.3,
    aes(ymax = pmax(upper, 0), ymin = lower, x = Time),
    fill = "pink",
    alpha = 0.5
  ) +
  ggplot2::geom_line(
