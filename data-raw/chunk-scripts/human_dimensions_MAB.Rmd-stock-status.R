...
stock_status <- ecodata::stock_status %>%
  dplyr::mutate(
    Code = recode(Code, "Dogfish" = "Sp. Dogfish"),
...
ggplot2::ggplot(data = stock_status) +
  ...
  ggplot2::scale_color_brewer(palette = "Dark2", breaks = stock_status$score) +
  ggplot2::ylim(0, y.max) +
  ggplot2::xlim(0, x.max) +
  ggplot2::geom_text(
    data = unknown,
    aes(x = x - 0.5, y = y + 0.2, label = text), #Custom legend for unknown stock status
    size = c(4.75, rep(4, 6))
  ) +
