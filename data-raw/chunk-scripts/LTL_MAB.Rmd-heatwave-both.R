cumu <- ecodata::heatwave %>%
  ...
  dplyr::mutate(
    Var = dplyr::recode(
      Var,
      "cumulative intensity-Surface" = "Cumulative Intensity (degree C x days)"
    )
  )
...
maxin <- ecodata::heatwave %>%
  ...
  dplyr::mutate(
    Var = dplyr::recode(
      Var,
      "maximum intensity-Surface" = "Maximum Intensity (degree C)"
    )
  )
...
cumud <- ecodata::heatwave %>%
  ...
  dplyr::mutate(
    Var = dplyr::recode(
      Var,
      "cumulative intensity-SurfaceDetrended" = "Cumulative Intensity Detrended (degree C x days)"
    )
  )
...
maxind <- ecodata::heatwave %>%
  ...
  dplyr::mutate(
    Var = dplyr::recode(
      Var,
      "maximum intensity-SurfaceDetrended" = "Maximum Intensity Detrended (degree C)"
    )
  )
...
mab.hw %>%
  ...
  ecodata::theme_ts() +
  ggplot2::theme(strip.text = element_text(hjust = 0, face = "italic")) +
  ecodata::theme_title()
