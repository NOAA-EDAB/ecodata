df.symbol <- ecodata::wind_port %>%
  filter(
    EPU == "NE",
    !Var %in% c("wea_rev", "total_rev", "perc_rev_min", "perc_rev_max")
  ) %>%
  ...
  filter(!value == "NA") %>%
  dplyr::mutate(
    symbol = recode(Variable, EJ = -7, Gentrification = -3),
    Variable = recode(
      Variable,
      "EJ" = "Mid-High to High EJ Concerns",
...
df.all.perc <- ecodata::wind_port %>%
  filter(EPU == "NE") %>%
  pivot_wider(names_from = Var, values_from = Value) %>%
  dplyr::mutate(ordering = wea_rev, City = paste0(City, State)) %>%
  pivot_longer(
    cols = c(perc_rev_min, perc_rev_max, total_rev),
    names_to = "Var",
    values_to = "Value"
  ) %>%
  ...
  dplyr::mutate(
    Var = recode(
      Var,
      "perc_rev_min" = "WEA Revenue",
      "perc_rev_max" = "WEA Revenue Min-Max Range",
      "total_rev" = "Non-WEA Revenue"
    ),
    Var = factor(
      Var,
      levels = c("Non-WEA Revenue", "WEA Revenue Min-Max Range", "WEA Revenue")
    )
  )
p2 <- ggplot2::ggplot() +
  ggplot2::geom_bar(
    data = df.all.perc,
    aes(fill = Var, y = reorder(City, ordering), x = Value),
    stat = "identity"
  ) +
  scale_fill_brewer() +
  ggplot2::theme(
  ...
  ggplot2::geom_point(
    data = df.symbol,
    aes(x = symbol, y = City, shape = Variable)
  ) +
