month <- seq(as.Date("2021-01-01"), as.Date("2021-12-31"), by = "1 month")
...
phyto_year_nano <- ecodata::phyto_size %>%
  dplyr::filter(
    EPU == c("GB"),
    Var %in%
      c("WEEKLY_NANO_PERCENTAGE_MEDIAN", "WEEKLY_MICRO_PERCENTAGE_MEDIAN")
  ) %>%
  ...
  group_by(Time) %>%
  dplyr::mutate(
    nano = as.numeric(WEEKLY_NANO_PERCENTAGE_MEDIAN[1]) +
  ...
  dplyr::filter(year == 2021, !Value == "NA") %>%
  dplyr::mutate(Value = Value * 100)


phyto_year_micro <- ecodata::phyto_size %>%
  dplyr::filter(EPU == c("GB"), Var == c("WEEKLY_MICRO_PERCENTAGE_MEDIAN")) %>%
  mutate(Value = as.numeric(Value)) %>%
  tidyr::separate(., Time, into = c("Cat", "WEEK"), sep = "_") %>%
  dplyr::mutate(
    year = stringr::str_sub(WEEK, 1, 4),
...
p <- ggplot2::ggplot() +
  geom_area(
    aes(
      ...
      fill = factor(
        out_phyto$Var,
        c(
          "CLIMATOLOGICAL_WEEK_PICO_PERCENTAGE_MEDIAN",
          "CLIMATOLOGICAL_WEEK_NANO_PERCENTAGE_MEDIAN",
          "CLIMATOLOGICAL_WEEK_MICRO_PERCENTAGE_MEDIAN"
        )
      )
    ),
    alpha = 0.6
  ...
  ggplot2::geom_line(
    aes(x = as.numeric(phyto_year_nano$wk), y = phyto_year_nano$Value),
    color = "#FC8D62",
    size = 1.5
  ) +
  ggplot2::geom_line(
    aes(x = as.numeric(phyto_year_micro$wk), y = phyto_year_micro$Value),
    color = "#66C2A5",
    size = 1.5
  ) +
...
phyto_year_micro_gom <- ecodata::phyto_size %>%
  dplyr::filter(EPU == c("GOM"), Var == c("WEEKLY_MICRO_PERCENTAGE_MEDIAN")) %>%
  mutate(Value = as.numeric(Value)) %>%
  tidyr::separate(., Time, into = c("Cat", "WEEK"), sep = "_") %>%
  dplyr::mutate(
    year = stringr::str_sub(WEEK, 1, 4),
...
p2 <- ggplot2::ggplot() +
  geom_area(
    aes(
      x = as.numeric(out_phyto_gom$WEEK),
      y = out_phyto_gom$Value,
      fill = factor(
        out_phyto_gom$Var,
        c(
          "CLIMATOLOGICAL_WEEK_PICO_PERCENTAGE_MEDIAN",
  ...
  #ggplot2::geom_point(data = chlor, aes(x = as.numeric(WEEK), y = Value)) +
  ggplot2::geom_line(
    aes(x = as.numeric(phyto_year_nano_gom$wk), y = phyto_year_nano_gom$Value),
    color = "#FC8D62",
    size = 1.5
  ) +
  ggplot2::geom_line(
    aes(
      x = as.numeric(phyto_year_micro_gom$wk),
      y = phyto_year_micro_gom$Value
    ),
    color = "#66C2A5",
    size = 1.5
  ) +
...
plot_w_title <- cowplot::plot_grid(
  title,
  plot_row,
  ncol = 1,
  rel_heights = c(0.1, 1)
