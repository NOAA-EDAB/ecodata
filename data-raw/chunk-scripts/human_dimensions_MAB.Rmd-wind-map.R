nesbath <- marmap::fortify.bathy(marmap::getNOAA.bathy(
  lon1 = -77,
  lon2 = -65,
  lat1 = 35,
  lat2 = 45,
...
lease_s <- sf::st_read(file.path(
  gis.dir,
  'BOEM lease areas/ne_existing_leasesSPoly.shp'
))
lease_n <- sf::st_read(file.path(
  gis.dir,
  'BOEM lease areas/ne_existing_leasesNPoly.shp'
))
lease_m <- sf::st_read(file.path(
  gis.dir,
  'BOEM lease areas/ne_existing_leasesMPoly.shp'
))
prop_n <- sf::st_read(file.path(
  gis.dir,
  'BOEM lease areas/ne_proposed_leases_NPoly.shp'
))
prop_m <- sf::st_read(file.path(
  gis.dir,
  'BOEM lease areas/ne_proposed_leases_MPoly.shp'
))
...
ggplot2::ggplot() +
  ...
  ggplot2::geom_segment(
    aes(x = -74.6, y = 37.4, xend = -75.4, yend = 38),
    colour = "blue4"
  ) +
  ggplot2::geom_segment(
    aes(x = -71.1, y = 40.2, xend = -71.6, yend = 41.1),
    colour = "blue4"
  ) +
