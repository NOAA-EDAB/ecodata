...
get_group_mean <- function(season, ESP_name, anom = T) {
  #Import raster data
  season.chl <- raster::stack(file.path(raw.dir, paste0(season, "_chl.nc")))
  ...
  out <- raster::mask(season.chl, ESP_sf[ESP_sf$ID == ESP_name, ])

  #Find seasonal anomaly
  mean_chl <- NULL
  for (k in 1:nlayers(out)) {
    chl <- mean(out[[k]]@data@values, na.rm = T)
    # var <- season
    # year = out@data@names[i]
    df <- data.frame(Value = chl, year = 1997 + k, ESP = ESP_name, Var = season)
    mean_chl <- df %>% rbind(mean_chl)
...
for (e in stock_list2) {
  ...
  for (i in season_list) {
    assign(
      e,
      rbind(get(e), get_group_mean(season = i, ESP_name = e, anom = TRUE))
    )
  }
}
...
usethis::use_data(ESP_seasonal_chl, overwrite = TRUE)
