...
get_seasonal_ltm <- function(file) {
  ltm <- raster::stack(file) %>%
    raster::crop(extent(280, 300, 30, 50)) %>%
    raster::rotate()
...
get_group_mean <- function(fname, epu_name, anom = TRUE) {

  # Import raster data
  raw <- raster::stack(file.path(fname))
  ...
  for (i in 1:nlayers(out)) {
    if (anom) {
      ...
      sst <- mean(out[[i]]@data@values, na.rm = T) - seasonal_epu_ltm(
        ltm = get(paste0(season, ".ltm")),
        epu = epu_name
      )
      var <- "anomaly"
    } else {
...
# process data ----
get_seasonal_ltm(file = here::here("data-raw", "gridded", "sst.day.mean.ltm.1982-2010.nc"))
seasonal_oisst_anom <- finish_sst(fname = here::here("data-raw", "gridded", "sst_data"))

# metadata ----
# seasonal_oisst_anom_nc does not exist as a global variable anymore - update
