...
get_zoo_abundance <- function(save_clean = F) {
  zoo_abundance_anom <- readxl::read_excel(file.path(
    raw.dir,
    zoo_abundance_anom_xlsx
  )) %>%
    dplyr::mutate(Var = paste0(Taxa), EPU = Region, Time = Year) %>%
...
get_zoo_strat_abun <- function(save_clean = F) {
  zoo_strat_abun <- readxl::read_excel(file.path(
    raw.dir,
    zoo_strat_abun_xlsx
  )) %>%
