...
get_wind <- function(save_clean = F) {
  d <- read.csv(file.path(
    raw.dir,
    "NCEP NARR surface wind; TKE; HLCY, monthly, 1979-2018, V1.csv"
  ))

  # Processing steps for all wind speed data
  wind_clean1 <- d %>%
    gather(., Var, Value, GB.uwnd:MAB.tke) %>% #convert wide to long
    dplyr::rename(Time = Month.Year) %>% #rename time variable
    tidyr::separate(Var, c("EPU", "Var"), "\\.") %>% #separate out EPU from variable names
    dplyr::mutate(
      Time = dmy(.$Time), #Convert to date format
      Units = plyr::mapvalues(
        Var,
        from = unique(Var),
        to = c(rep("J/kg", 2), "m^2/sec^2", "J/kg")
      ), #add units
      Time,
      season = plyr::mapvalues(
        month(Time),
        from = seq(1, 12, 1), #Get season
        to = c(
          rep("winter", 3),
          rep("spring", 3),
          rep("summer", 3),
  ...
  total_wind_speed <- wind_clean1 %>%
    splyr::filter(Var == "uwnd" | Var == "vwnd") %>% #select variables
    tidyr::spread(., Var, Value) %>% #convert to wide for calculating tws
    dplyr::mutate(`total wind speed` = sqrt(uwnd^2 + vwnd^2)) %>% #tws
    dplyr::select(-uwnd, -vwnd) %>% #start processing back to SOE format
    tidyr::gather(., Var, Value, `total wind speed`) #convert to long

  wind_clean <- rbind(wind_clean1, total_wind_speed)
  ne_wind <- wind_clean %>%
    tidyr::unite(., Var, c(Var, season), sep = " ") %>% #merge season into Var column
