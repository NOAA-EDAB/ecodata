...
get_ESP_gsi <- function(save_clean = F) {
  ESP_gsi_season <- readxl::read_excel(file.path(raw.dir, gsi_xlsx)) %>%
    ...
    dplyr::mutate(
      ...
      year = case_when(month == "12" ~ (year + 1), month != "12" ~ year)
    ) %>%
    group_by(year, season_month) %>%
    dplyr::summarise(Value = mean(Value)) %>%
    dplyr::mutate(
      Var = "gulf stream index - season",
  ...
  attr(
    ESP_gsi,
    "tech-doc_url"
  ) <- "https://noaa-edab.github.io/tech-doc/gulf-stream-index.html"
