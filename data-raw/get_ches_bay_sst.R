...
get_ches_bay_sst <- function(save_clean = F) {
  ches1 <- raster::raster(
    file.path(raw.dir, ches_1_nc),
    varname = "sst_anomaly"
  ) #Winter
  ches2 <- raster::raster(
    file.path(raw.dir, ches_2_nc),
    varname = "sst_anomaly"
  ) #Spring
  ches3 <- raster::raster(
    file.path(raw.dir, ches_3_nc),
    varname = "sst_anomaly"
  ) #Summer
  ches4 <- raster::raster(
    file.path(raw.dir, ches_4_nc),
    varname = "sst_anomaly"
  ) #Fall
  ...
  ches_bay_sst <- r %>%
    dplyr::rename(
      Winter = "sea_surface_subskin_temperature.anomaly.2025.minus.2007.2024.1",
    ...
    dplyr::rename(Longitude = "y", Latitude = "x", Var = "Season") %>%
    filter(!Value == "NaN")
