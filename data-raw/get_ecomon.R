...
get_ecomon <- function(save_clean = F) {
  d <- read.csv(
    file.path(raw.dir, "EcoMon Nutrient Data Through June 2018.csv"),
  ...
  d1 <- d %>%
    ...
    gather(
      ...
      -Depth_sampling,
      -Depth_station
    ) %>% #turn wide to long while retaining lat/lon
  ...
  ecomon <- d1 %>%
    ...
    filter(
      ...
      !is.na(EPU),
      !Var %in%
        c("BTLNBR", "CASTNO", "Depth_sampling", "Depth_station", "STNNBR")
    ) %>%
