...
reportResults <- function(
  ...
  printString <- paste(
    "\nYears fit:",
    startyear,
    "to 2021. The model is fitted with",
    sum(!is.na(dat)),
    "non-missing counts. ",
    addlstring
  )
  ...
  if (loglikdiff < 0.01 & aicdiff < 0.01) {
    printString <- paste0(
      printString,
      "\nU = ",
      signif(model$par$U, 3),
      ", Q = ",
      signif(model$par$Q, 3)
    )
    if (length(model$par$R) > 0) {
      # R only exists in the model if it was estimated and not fixed
      printString <- paste0(
        printString,
        ", R = ",
        signif(model$par$R, 3),
        ".\n"
      )
    } else {
      printString <- paste0(
    ...
    printString <- paste(
      printString,
      "Log likelihood and/or AIC vary more than 1% in runs",
...
dat <- read.csv(file.path(here::here(
  "data-raw/pupcountsalldatraw_21-final2021.csv"
)))
if ("minCount" %in% colnames(dat)) {
  dat$minCount[which(is.na(dat$minCount))] <- dat$count[which(is.na(
    dat$minCount
  ))]
}
if ("maxCount" %in% colnames(dat)) {
  dat$maxCount[which(is.na(dat$maxCount))] <- dat$count[which(is.na(
    dat$maxCount
  ))]
}
...
datw <- reshape(
  dat[which(dat$colony %in% names(which(table(dat$colony) >= minNYears))), ],
  idvar = "colony",
  timevar = "Year",
  direction = "wide"
)
...
if (length(missingyrs) > 0) {
  # fill in NA for years missed at all sites
...
dateFrom <- data.frame(
  colony = c("Muskeget", "Monomoy", "Green", "Seal", "Nomans Land"),
...
datlog <- data.frame(t(datwmatlogMax)) %>%
  dplyr::mutate(Time = 1988:2021, Var2 = c("log")) %>%
  tidyr::pivot_longer(
    cols = c("Seal", "Monomoy", "Muskeget", "Green", "Nomans.Land"),
    names_to = "Var",
    values_to = "Value"
  )
datw <- datw %>%
...
datcount <- data.frame(t(datw)) %>%
  dplyr::mutate(Time = 1988:2021, Var2 = c("count")) %>%
  tidyr::pivot_longer(
    cols = c("Seal", "Monomoy", "Muskeget", "Green", "Nomans.Land"),
    names_to = "Var",
    values_to = "Value"
...
seal_pups <- rbind(datlog, datcount) %>%
  dplyr::mutate(Var = paste0(Var2, "-", Var), EPU = c("NE")) %>%
  dplyr::select(!Var2)
