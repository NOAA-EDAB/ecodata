...
get_surface_temp_mom6 <- function(save_clean = F) {
  surface_temp_mom6_forecast <- read.csv(file.path(
    raw.dir,
    surface_temp_mom6_forecast_csv
  )) |>
  ...
  surface_temp_mom6_hindcast <- read.csv(file.path(
    raw.dir,
    surface_temp_mom6_hindcast_csv
  )) |>
    dplyr::mutate(
      Season = dplyr::recode(
        Season,
  ...
  surface_temp_mom6 <- rbind(
    surface_temp_mom6_forecast,
