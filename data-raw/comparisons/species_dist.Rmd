...
# Replace 'species_dist' with intended dataset
dataset<-ecodata::species_dist
dataset24<-ecodata25::species_dist

# Define dataset-specific arguments for plot functions
report<-list("MidAtlantic","NewEngland")
variable<-list("along","depth")
...
ecodata::plot_species_dist(report = report[1], n = 10, varName = variable[1]) + 
  ggplot2::ggtitle("SOE 2026") + ggplot2::theme(plot.title = ggplot2::element_text(size = 20))

ecodata25::plot_species_dist(report = report[1], n = 10, varName = variable[1]) + 
  ggplot2::ggtitle("SOE 2025") + ggplot2::theme(plot.title = ggplot2::element_text(size = 20))
```

## `r paste(report[1],variable[2])`
```{r, fig.show="hold", out.width="50%", echo=FALSE, message=FALSE, warning=FALSE}
ecodata::plot_species_dist(report = report[1], n = 10, varName = variable[2]) + 
  ggplot2::ggtitle("SOE 2026") + ggplot2::theme(plot.title = ggplot2::element_text(size = 20))

ecodata25::plot_species_dist(report = report[1], n = 10, varName = variable[2]) + 
  ggplot2::ggtitle("SOE 2025") + ggplot2::theme(plot.title = ggplot2::element_text(size = 20))
```
