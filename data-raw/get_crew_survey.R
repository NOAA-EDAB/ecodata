...
get_crew_survey <- function(save_clean = F) {
  crew_survey_orig <- readxl::read_xlsx(
  ...
  crew_survey = crew_survey_orig |>
    ...
    dplyr::mutate(
      ...
      Var = dplyr::case_when(
        ...
        Var.long == '[Your actual earnings] Job satisfaction Combined' ~
          'Earnings_Satisfaction',
        Var.long == '[Predictability of earnings] Job satisfaction Combined' ~
          'Predicatable_Earnings_Satisfaction',
        Var.long == '[Job safety] Job satisfaction Combined' ~
          'Safety_Satisfaction',
        Var.long == '[Time away] Job satisfaction Combined' ~
          'Time_Away_Satisfaction',
        Var.long == '[Physical fatigue] Job satisfaction Combined' ~
          'Physical_Fatigue_Satisfaction',
        Var.long == '[Healthfulness] Job satisfaction Combined' ~
          'Healthfulness_Satisfaction',
        Var.long == '[Adventure] Job satisfaction Combined' ~
          'Adventure_Satisfaction',
        Var.long == '[Challenge] Job satisfaction Combined' ~
          'Challenge_Satisfaction',
        Var.long == '[Own boss] Job satisfaction Combined' ~
          'Own_Boss_Satisfaction',
        Var.long ==
          'Years in commercial fishing (0 if less than a year) Combined' ~
          'Years_in_Fishing',
        TRUE ~ Var.long
    ...
    tidyr::pivot_longer(
      cols = -c(Time, EPU, Var.long, Var, Var.Answer),
      names_to = "Metric",
      values_to = "Value",
