...
get_stock_heatwave <- function(stock) {
  ...
  dat <- data.frame(value) %>%
    ...
    dplyr::mutate(t = substr(t, 2, 11), t = lubridate::as_date(t)) %>%
    dplyr::select(t, temp)
  dat <- as.tibble(dat)
  ts <- heatwaveR::ts2clm(
    dat,
...
atlantic_menhaden_north_spring <- get_stock_heatwave(
  "atlantic_menhaden_north_spring"
)
atlantic_menhaden_north_fall <- get_stock_heatwave(
  "atlantic_menhaden_north_fall"
)
atlantic_menhaden_south_fall <- get_stock_heatwave(
  "atlantic_menhaden_south_fall"
)
atlantic_menhaden_south_spring <- get_stock_heatwave(
...
northern_shortfin_squid_both <- get_stock_heatwave(
...
windowpane_flounder_gom_gb_both <- get_stock_heatwave(
  "windowpane_flounder_gom_gb_both"
)
windowpane_flounder_sne_ma_both <- get_stock_heatwave(
...
yellowtail_flounder_cc_spring <- get_stock_heatwave(
...
yellowtail_flounder_sne_ma_spring <- get_stock_heatwave(
  "yellowtail_flounder_sne_ma_spring"
)
yellowtail_flounder_sne_ma_fall <- get_stock_heatwave(
...
cum.intensity <- rbind(
  ...
  dplyr::mutate(
    Time = as.numeric(format(as.Date(date_start, format = "%Y-%m-%d"), "%Y"))
  ) %>%
...
ESP_heatwave <- rbind(cum.intensity, max.intensity) %>%
  dplyr::mutate(Units = "degrees C", Time = as.numeric(Time))
...
sf_cod <- rgdal::readOGR(
  file.path(here::here("data-raw", "ESP"), "codstox.shp"),
  verbose = F
)
crs(sf_cod) <- crs
sf <- as(sf_cod, "sf") %>%
...
sst10 <- raster::stack(
  file.path(here::here(raw.dir, "sst.day.mean.2012.v2.nc")),
