...
winter.ltm <- ltm[[1:90]]
winter.ltm <- raster::stackApply(
  winter.ltm,
  indices = rep(1, nlayers(winter.ltm)),
  mean
)

spring.ltm <- ltm[[91:181]]
spring.ltm <- raster::stackApply(
  spring.ltm,
  indices = rep(1, nlayers(spring.ltm)),
  mean
)

summer.ltm <- ltm[[182:273]]
summer.ltm <- raster::stackApply(
  summer.ltm,
  indices = rep(1, nlayers(summer.ltm)),
  mean
)

fall.ltm <- ltm[[274:365]]
fall.ltm <- raster::stackApply(
  fall.ltm,
  indices = rep(1, nlayers(fall.ltm)),
  mean
)
...
get_group_mean <- function(fname, ESP_name, anom = T) {
  ...
  for (i in 1:nlayers(raw)) {
    assign(
      "year",
      rbind(year, data.frame(Time = str_extract(raw[[i]]@data@names, "\\d{4}")))
    )
  }
  ...
  year_split <- year %>%
    ...
    dplyr::mutate(
      season = ifelse(
        ...
        "winter",
        ifelse(
          day > 90 & day <= 181 & leap == "common",
          "spring",
          ifelse(
            day > 181 & day <= 273 & leap == "common",
            "summer",
            ifelse(
              day > 273 & leap == "common",
              "fall",
              ...
              ifelse(
                ...
                "winter",
                ifelse(
                  day > 91 & day <= 181 & leap == "leap",
                  "spring",
                  ifelse(
                    day > 181 & day <= 273 & leap == "leap",
                    "summer",
  ...
  for (i in 1:nlayers(out)) {
    if (anom) {
      ...
      sst <- mean(out[[i]]@data@values, na.rm = T) -
        seasonal_ESP_ltm(ltm = get(paste0(season, ".ltm")), ESP = ESP_name)
      var <- "anomaly"
    } else {
      sst <- mean(out[[i]]@data@values, na.rm = T)
      var <- "absolute"
...
for (e in stock_list2) {
  ...
  for (i in 1:4) {
    ...
    assign(
      e,
      rbind(get(e), get_group_mean(fname = fname[i], ESP_name = e, anom = TRUE))
    )
  }
}

#process output
ESP_seasonal_oisst_anom <- rbind(
  acadian_redfish_both,
  alewife_both,
  american_lobster_gb_both,
  american_lobster_gom_both,
  ...
  dplyr::mutate(
    Time = as.numeric(stringr::str_extract(year, "\\d{4}")),
    Var = paste(
      stringr::str_extract(year, "winter|spring|summer|fall"),
      "OI SST Anomaly"
    )
  ) %>%
  dplyr::select(-year) %>%
