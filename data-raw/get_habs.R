...
get_habs <- function(save_clean = F) {
  ...
  me_psp2 <- me_psp %>%
    ...
    dplyr::rename(Time = Year, Value = PSP_Exceed_Threshold_.) %>%
    dplyr::mutate(Var = c("ME"))

  nh_psp <- read.csv(
    file.path(raw.dir, psp_nh_csv),
    header = TRUE,
    stringsAsFactors = FALSE
  )
  nh_psp2 <- nh_psp %>%
    dplyr::select(Year, PSP.Exceed.Threshold..) %>%
    dplyr::rename(Time = Year, Value = PSP.Exceed.Threshold..) %>%
    dplyr::mutate(Var = c("NH"))

  ma_psp <- read.csv(
    file.path(raw.dir, psp_ma_csv),
    header = TRUE,
    stringsAsFactors = FALSE
  )
  ma_psp2 <- ma_psp %>%
    dplyr::select(Year, PSP.) %>%
    dplyr::rename(Time = Year, Value = PSP.) %>%
