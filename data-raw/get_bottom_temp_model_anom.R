...
get_bottom_temp_model_anom <- function(save_clean = F){
  ...
  bottom_temp_model_anom_annual <- readRDS(file.path(raw.dir,btannual_input))%>%
    dplyr::arrange(Source,Time,EPU,Var)

  bottom_temp_model_anom <- rbind(bottom_temp_model_anom, bottom_temp_model_anom_annual)
