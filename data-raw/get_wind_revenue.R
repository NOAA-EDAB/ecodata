...
get_wind_revenue <- function(save_clean = F) {
  ne_wind_revenue <- readxl::read_excel(
    file.path(raw.dir, wind_rev_xlsx),
  ...
  mab_wind_revenue <- readxl::read_excel(
    file.path(raw.dir, wind_rev_xlsx),
  ...
  wind_revenue <- ne_wind_revenue %>%
    ...
    dplyr::mutate(
      Species = dplyr::recode(Species, "Atlantic Surfclam" = "Surfclam")
    ) %>%
    dplyr::mutate(
      Species = dplyr::recode(Species, "Atlantic Herring" = "Atlantic herring")
    ) %>%
    dplyr::mutate(
      Species = dplyr::recode(Species, "Atlantic Halibut" = "Atlantic halibut")
    ) %>%
    dplyr::mutate(
      Species = dplyr::recode(Species, "Atlantic Cod" = "Atlantic cod")
    ) %>%
    dplyr::mutate(
      Species = dplyr::recode(Species, "Witch Flounder" = "Witch flounder")
    ) %>%
    dplyr::mutate(
      Species = dplyr::recode(Species, "Winter Flounder" = "Winter flounder")
    ) %>%
    dplyr::mutate(
    ...
    dplyr::mutate(Value = as.double(Value), Time = as.integer(Time)) %>%
