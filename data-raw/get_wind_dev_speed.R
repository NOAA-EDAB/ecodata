...
get_wind_dev_speed <- function(save_clean = F) {
  wind_dev_speed_2021 <- read_excel(
    file.path(raw.dir, wind_dev_speed_xlsx_2021),
    sheet = 3
  ) %>%
    janitor::row_to_names(row_number = 1) %>%
    dplyr::rename(Time = "Year End", Value = "Project Acres") %>%
    dplyr::mutate(Var = c("Tot_Area_Acres"), EPU = c("ALL")) %>%
    dplyr::select(Time, Value, Var, EPU) %>%
    dplyr::filter(!Value == "NA") %>%
    dplyr::mutate(Report_year = c("year2021"))

  wind_dev_speed_2022 <- read_excel(
    file.path(raw.dir, wind_dev_speed_xlsx_2022),
    sheet = 2
  ) %>%
    dplyr::rename(
      Time = "Turbines in the water (construction year)",
    ...
    dplyr::filter(
      !Time == "Construction is estimated to be 2 years for each project."
    ) %>%
    tidyr::pivot_longer(
      cols = c(
        Tot_Area_Acres,
  ...
  wind_dev_speed_2023 <- read_excel(file.path(
    raw.dir,
    wind_dev_speed_xlsx_2023
  )) %>%
    dplyr::rename(
      Tot_Area_Acres = Acres,
      Num_Foundations = Fndns,
      Cable_Miles = Cab_Total
    ) %>%
    tidyr::pivot_longer(
      cols = c(
        "Tot_Area_Acres",
    ...
    dplyr::mutate(
      Time = as.numeric(Const_Yr),
      EPU = c("All"),
      Report_year = c("year2023")
    ) %>%
    dplyr::select(!Const_Yr)

  wind_dev_speed_2024 <- read_excel(file.path(
    raw.dir,
    wind_dev_speed_xlsx_2024
  )) %>%
    dplyr::rename(
      Tot_Area_Acres = Acres,
      Num_Foundations = Fndns,
