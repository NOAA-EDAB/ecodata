...
field <- "stratification (0-50 m) SS"
...
get_dat <- function(field){
  ...
  if (all(is.na(as.numeric(Value))) | sd(Value, na.rm = T) == 0){
    Value <- NA #Assign as NA is not including
  } else {
...
env_wide <- norm_env %>% #Clean up output
  ...
  group_by(Var) %>% 
  mutate(count = n()) %>% 
  filter(count > 30, !str_detect(Var,"reference"), str_detect(Var,"MAB")) %>% #select only data from MAB that has over 30 data points (interpolated or otherwise)
  dplyr::select(-count) %>%
  spread(., Var, Value) 
...
env_wide <- norm_env %>% #Clean up output
  ...
  group_by(Var) %>% 
  mutate(count = n()) %>% 
  filter(count > 30, !str_detect(Var,"reference"), str_detect(Var,"GB")) %>% #select only data from GB that has over 30 data points (interpolated or otherwise)
  dplyr::select(-count) %>%
  spread(., Var, Value) 
...
env_wide <- norm_env %>% #Clean up output
  ...
  group_by(Var) %>% 
  mutate(count = n()) %>% 
  filter(count > 30, !str_detect(Var,"reference"), str_detect(Var,"GOM")) %>% #select only data from GOM that has over 30 data points (interpolated or otherwise)
  dplyr::select(-count) %>%
  spread(., Var, Value) 
