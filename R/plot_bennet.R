...
plot_bennet <- function(
  ...
  if (varName == "guild") {
    ...
    indicators <- indicators |>
      ...
      dplyr::filter(!Var == "Revenue", !Guild == "Total", !Time < 1981) |>
    ...
    p <- ggplot2::ggplot() +
      ...
      ggplot2::geom_bar(
        data = indicators,
        ggplot2::aes(x = Time, y = Value, fill = Guild),
        stat = "identity"
      ) +
      #ggplot2::scale_fill_manual(name = "Indicators", values = Guild) +
      ggplot2::geom_line(
        data = indicators,
        ggplot2::aes(x = Time, y = component, color = "$"),
        linewidth = 0.7
      ) +
    ...
    indicators <- indicators |>
      dplyr::filter(
        stringr::str_detect(Var, pattern = "Total"),
    ...
    p <- ggplot2::ggplot() +
      ...
      ggplot2::geom_bar(
        data = indicators,
        ggplot2::aes(x = Time, y = Value, fill = Var),
        stat = "identity"
      ) +
      ggplot2::scale_fill_manual(name = "Indicators", values = ind_fill) +
      ggplot2::geom_line(
        data = revchange,
        ggplot2::aes(x = Time, y = Value, color = "$"),
        linewidth = 0.7
      ) +
