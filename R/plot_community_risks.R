...
plot_community_risks <- function(
  ...
  vulquants <- data.frame(
    ymin = c(1, 1.51, 2.51, 3.51),
    ymax = c(1.5, 2.5, 3.5, 4),
    category = c("Low", "Moderate", "High", "Very High")
  )
  ...
  if (plottype == "propcomm") {
    ...
    props <- ecodata::community_risks |>
      ...
      dplyr::filter(
        !is.na(Value), #remove communities with no ranking in a given year
        Var %in% varName,
        EPU %in% filterEPUs
      ) |>
      ...
      dplyr::mutate(catn = dplyr::n(), propcat = catn / commn)
    ...
    p <- ggplot2::ggplot() +
      ...
      ggplot2::geom_point(
        data = propsonly,
        ggplot2::aes(x = Time, y = propcat, color = risk)
      ) +
      ggplot2::geom_line(
        data = propsonly,
        ggplot2::aes(x = Time, y = propcat, color = risk)
    ...
    var_suffix <- dplyr::case_when(
      plottype == "regionrev" ~ "_region",
    ...
    regional <- ecodata::community_risks |>
      ...
      tidyr::separate(Var, into = c("RegionVar", "Var"), sep = "-") |>
      dplyr::select(-RegionVar) |>
      dplyr::distinct() |>
      dplyr::filter(
        Var == paste0(varName, var_suffix),
        EPU %in% filterEPUs #,
        # units didn't come through
        # Units %in% units
      )
