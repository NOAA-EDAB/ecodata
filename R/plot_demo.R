...
plot_demo <- function(shadedRegion = NULL, report = "MidAtlantic", n = 0) {
  ...
  epu_labels <- data.frame(
    EPU = c("Mid-Atlantic\n Bight", "Gulf of Maine", "Georges Bank"),
  ...
  epumap <- ggplot2::ggplot() +
    ...
    ggplot2::geom_text(
      data = epu_labels,
      ggplot2::aes(x = longitude, y = latitude, label = EPU),
      size = 1.7
    ) +
    ecodata::theme_map() +
    ggplot2::scale_x_continuous(
      breaks = seq(-78, -65, by = 4),
      expand = c(0.01, 0.01)
    ) +
  ...
  p <- ggpubr::ggarrange(psample, epumap, nrow = 1, ncol = 2)
