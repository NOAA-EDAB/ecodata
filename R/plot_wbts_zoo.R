...
#'
#' WBTS Zooplankton indicator provided by Runge Lab at the Univeristy of Maine
#'
#' @param shadedRegion Numeric vector. Years denoting the shaded region of the plot (most recent 10)
#' @param report Character string. Which SOE report (only accepts "NewEngland")
...
plot_wbts_zoo <- function(shadedRegion = NULL, report = "NewEngland", n = 0) {
  if (report != "NewEngland") {
    stop("This function is only available for the New England report.")
  }
  ...
  p <- #fix |>
    ...
    ggplot2::geom_line(
      data = wbts.zoo.model,
      ggplot2::aes(x = day_of_year, y = pred),
      color = "black"
    ) +
    ggplot2::geom_ribbon(
      data = wbts.zoo.model,
      ggplot2::aes(x = day_of_year, ymin = pred_low, ymax = pred_high),
      fill = "grey",
      alpha = 0.4
    ) +
    ...
    ggplot2::labs(
      ...
      title = "WBTS, Calanus Climatology"
    ) +
    ggplot2::geom_vline(
      xintercept = c(75.5, 147.5, 247.5, 364.5),
      linetype = "dashed",
      color = "black",
      linewidth = 1,
      alpha = 0.3
    ...
    ggplot2::guides(
      ...
      shape = ggplot2::guide_legend(title = ggplot2::element_blank()),
      size = ggplot2::guide_legend(title = ggplot2::element_blank())
    )
