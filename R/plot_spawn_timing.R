...
plot_spawn_timing <- function(
  ...
  filt <- fix |>
    dplyr::filter(Season == "SPRING", Var == varName) |>
    dplyr::mutate(
      Species = factor(Species, levels = c("Yellowtail", "Haddock")),
      Stock = factor(Stock, levels = c("GOM", "CC", "GB", "SNE")),
      plt_var = paste(Species, Stock)
    ) |>
    dplyr::mutate(
      plt_var = factor(
        plt_var,
  ...
  if (varName %in% c("MF", "meanTEMP", "meanJDAY")) {
    varLong <- dplyr::case_when(
      varName == "MF" ~ "Mature Females",
...
attr(plot_spawn_timing, "varName") <- c(
  "Resting",
  "Ripe",
  "Spent",
  "Developing",
