...
plot_habitat_diversity <- function(
  shadedRegion = NULL,
  report = "MidAtlantic",
  varName = "Diversity"
) {
  ...
  if (varName == "Diversity") {
    ...
    fix <- ecodata::habitat_diversity |>
      dplyr::filter(Var == varNames, EPU %in% filterEPUs) |>
      dplyr::group_by(EPU) |>
      dplyr::mutate(
    ...
    p <- ecodata::habitat_diversity |>
      ...
      ggplot2::ggtitle(paste(
        report,
        "Species Shannon Diversity from Habitat Assessment"
      )) +
    ...
    varNames <- "Richness"
    fix <- ecodata::habitat_diversity |>
      dplyr::filter(grepl(varNames, Var), EPU %in% filterEPUs) |>
    ...
    p <- ecodata::habitat_diversity |>
      ...
      ggplot2::geom_ribbon(
        ggplot2::aes(x = Time, ymax = upper, ymin = lower, color = EPU),
        alpha = setup$shade.alpha
      ) +
      ...
      ggplot2::geom_point(
        data = fix,
        ggplot2::aes(x = Time, y = mean, color = EPU)
      ) +
      ggplot2::geom_line(
        data = fix,
        ggplot2::aes(x = Time, y = mean, color = EPU),
      ...
      ggplot2::ggtitle(paste(
        report,
        "Species Richness from NEFSC Bottom Trawl Survey"
      )) +
