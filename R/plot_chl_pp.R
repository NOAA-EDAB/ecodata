  ...
  if (varName != "size") {
    ...
    if (plottype == "anomaly") {
      pp_anom <- ecodata::chl_pp |>
        dplyr::filter(
          stringr::str_detect(Var, "ANNUAL_PPD_RATIO_ANOMALY"),
          EPU %in% filterEPUs
        ) |>
    ...
    if (plottype == "weekly") {
      ...
      ltm_out <- ecodata::chl_pp |>
        ...
        dplyr::mutate(
          status = ifelse(
            Value < sd.high & Value > sd.low,
            "near_mean",
            ifelse(Value > sd.high, "high", ifelse(Value < sd.low, "low", NA))
          ),
          group = "PLOT"
        ) |>
        dplyr::filter(!Value == "NA")
    ...
    if (plottype == "weekly") {
      ...
      p <- ltm_out |>
        ...
        ggplot2::geom_ribbon(
          ggplot2::aes(x = Time, ymin = pmax(sd.low, 0), ymax = sd.high),
          alpha = 0.1,
          fill = "grey1"
        ) +
        ggplot2::geom_line(
    ...
    # which report? this may be bypassed for some figures
    if (plottype == "weekly") {
      p <- plot_phyto_size(report = report, EPU = EPU)
    } else {
...
# plot_chl_pp(plottype = "monthly", n = 28)
# plot_chl_pp(plottype = "monthly", report = "NewEngland", n = 28)
