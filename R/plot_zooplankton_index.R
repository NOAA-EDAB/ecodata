...
plot_zooplankton_index <- function(
  shadedRegion = NULL,
  report = "MidAtlantic",
  varName = "calfin",
  plottype = "index",
  ...
  if (
    ...
    stop(
      "varName should be one of the following: either Calfin, Euph, Smallcopesoe, Lgcopeall, Zoopvol, Smallcopeall"
    )
  }
  ...
  if (plottype == "index") {
    ...
    fix <- ecodata::zooplankton_index |>
      dplyr::filter(
        Var %in%
          c(
            paste("Fall", varName, "Abundance Index Estimate"),
    ...
    p <- ecodata::zooplankton_index |>
      ...
      ggplot2::geom_ribbon(
        ggplot2::aes(ymin = Lower, ymax = Upper, fill = Season),
        alpha = 0.5
      ) +
      ggplot2::geom_point() +
      ggplot2::geom_line() +
  ...
  if (plottype == "cog") {
    p <- ecodata::zooplankton_index |>
      dplyr::filter(
        Var %in%
          c(
            paste("Fall", varName, "Eastward Center of Gravity"),
      ...
      tidyr::separate(
        Var,
        into = c("Season", "A", "Direction", "B", "C", "D", "Var")
      ) |>
      dplyr::mutate(Var = tidyr::replace_na(Var, "Mean")) |>
      tidyr::pivot_wider(names_from = Var, values_from = Value) |>
      dplyr::mutate(
...
attr(plot_zooplankton_index, "varName") <- c(
  "Calfin",
  "Euph",
  "Smallcopesoe",
  "Lgcopeall",
