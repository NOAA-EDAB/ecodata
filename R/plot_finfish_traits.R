...
plot_finfish_traits <- function(
  ...
  p <- ecodata::finfish_traits |>
    tidyr::separate(Var, into = c("Season", "Var"), sep = "-") |>
    dplyr::filter(Var %in% c(varName), EPU %in% filterEPUs) |>
    dplyr::left_join(fix) |>
    ...
    ggplot2::geom_hline(
      ggplot2::aes(yintercept = hline, color = Season),
...
attr(plot_finfish_traits, "varName") <- c(
  "trophic_level",
  "offspring_size",
  "age_maturity",
  "length_maturity",
