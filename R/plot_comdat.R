  ...
  if (varName == "landings") {
    managed_landings <- ecodata::comdat |>
      dplyr::filter(
        Var %in%
          c(
            "Planktivore MAFMC managed species - Seafood Landings",
      ...
      dplyr::filter(stringr::str_detect(
        Var,
        paste0("JOINT|", setup$council_abbr)
      ))

    US_landings <- ecodata::comdat |>
      dplyr::filter(
        Var == "Seafood Landings", # There may be US landings that aren't seafood
        Time >= 1982
      )
    ...
    guilddat <- rbind(managed_landings_guild, total_landings) |>
      ...
      dplyr::mutate(
        #feeding.guild = stringr::str_extract(Var,c(feeding.guilds)),
    ...
    landings_agg <- rbind(
      total_landings_agg,
      managed_landings_agg,
      us_total_landings_agg
    ) # |>
  ...
  if (varName == "revenue") {
    ...
    rev_managed <- ecodata::comdat |>
      dplyr::filter(
        Var %in%
          c(
            "Piscivore MAFMC managed species - Revenue",
    ...
    rev_total <- ecodata::comdat |>
      dplyr::filter(Var == "Revenue", Time >= 1982) |>
    ...
    totdat <- rbind(rev_agg, rev_total) |>
      dplyr::mutate(Value = Total, Var = Status) |>
      dplyr::filter(Time >= 1982) |>
      dplyr::filter(EPU %in% filterEPUs)
  ...
  if (plottype == "guild") {
    ...
    p <- guilddat |>
      ...
      ggplot2::scale_color_manual(
        values = setup$series.col,
        aesthetics = "color"
      ) +
      ggplot2::geom_hline(
        ggplot2::aes(yintercept = hline, color = grouping, size = grouping),
        linewidth = setup$hline.size,
        alpha = setup$hline.alpha,
        linetype = setup$hline.lty
      ) +
