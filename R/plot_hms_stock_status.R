...
plot_hms_stock_status <- function(shadedRegion = NULL, report = "MidAtlantic") {
  ...
  unknownp <- data.frame(
    text = c("Unknown Status", "ATL SBH", "SPF", "WA BFT"),
    x = rep(0.9 * x.max, 4),
    y = seq(0.88 * y.max, 3.5, -0.3)
  )
  ...
  stock_status <- ecodata::hms_stock_status |>
    ...
    dplyr::group_by(spp) |>
    dplyr::filter(!species_abr == "------", !species_abr == "-----") |>
    dplyr::mutate(
  ...
  p <- stock_status |>
    ...
    ggplot2::geom_text(
      data = unknownp,
      ggplot2::aes(x = x, y = y, label = text), #Custom legend for unknown stock status
      size = c(4.75, rep(4, 3))
    ) +
    ggplot2::annotate(
      geom = "text",
      x = 0.43,
      y = 5,
      label = "ATL SBN (F/Fmsy = 22.5)",
      color = "#D95F02"
    ) +
    ggplot2::scale_color_brewer(
      palette = "Dark2", #Change legend labels for clarity
