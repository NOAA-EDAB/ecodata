...
plot_zoo_diversity <- function(
  ...
  if (report == "MidAtlantic") {
    ...
    filterEPUs <- c("GB", "GOM")
    to_string <- ggplot2::as_labeller(c(
      "GB" = "GB Zooplankton Diversity",
      "GOM" = "GOM Zooplankton Diversity"
    ))
  }
  ...
  fix <- ecodata::zoo_diversity |>
    dplyr::filter(EPU %in% filterEPUs, Time > 1991)

  mn <- fix |>
    dplyr::group_by(EPU, Var) |>
    dplyr::summarise(mn = mean(Value, na.rm = T), .groups = "drop")
