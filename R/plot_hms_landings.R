...
plot_hms_landings <- function(
  ...
  apex <- ecodata::hms_landings |>
    ...
    dplyr::group_by(Var) |>
    dplyr::mutate(
      Value = ifelse(stringr::str_detect(Var, varName), Value / 1000000, Value),
      hline = mean(Value)
    )

  ylabdat <- ifelse(
    varName == "Revenue",
    expression("Revenue (10"^6 * "USD)"),
  ...
  p <- apex |>
    ...
    ggplot2::geom_hline(
      ggplot2::aes(yintercept = hline, color = Var, size = Var),
