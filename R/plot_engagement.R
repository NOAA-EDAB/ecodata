...
plot_engagement <- function(
  ...
  if (varName == "Commercial") {
    eng <- ecodata::engagement |>
      ...
      dplyr::mutate(
        Town = dplyr::recode(Town, "OTHER,VA" = "OTHER,VA (includes REEDVILLE)")
      ) |>
  ...
  if (varName == "Recreational") {
    ...
    p <- eng.plot |>
      ...
      ggplot2::scale_color_manual(
        values = c("gray", "skyblue", "slateblue3", "navy"),
      ...
      ggplot2::ylab(expression(paste(
        "Population Relative Engagement Index",
        italic(" (factor score)")
      ))) +
      ...
      ggplot2::guides(
        color = ggplot2::guide_legend(title = "Engagement Score Rank")
      ) +
      ggrepel::geom_label_repel(
        ggplot2::aes(
          label = ifelse(Town %in% c(topTowns), as.character(Town), '')
        ),
        size = 2,
        force = 2.5,
        box.padding = 0.5,
