...
plot_slopewater <- function(shadedRegion = NULL, report = "NewEngland", n = 0) {
  ...
  fix <- ecodata::slopewater |>
    ...
    dplyr::mutate(
      ...
      Var = plyr::mapvalues(
        Var,
        from = c("WSW proportion ne channel", "LSLW proportion ne channel"),
        to = c("WSW", "LSW")
      )
    ) |>
  ...
  p <- fix |>
    ...
    ggplot2::geom_hline(
      ggplot2::aes(yintercept = hline, color = Origin),
