...
#'
#' plots thermal_habitat_area dataset.
#'
...
plot_thermal_habitat_area <- function(shadedRegion = NULL,
  ...
  p <- fix |>
    ggplot2::ggplot()+
    ggplot2::geom_ribbon(data=fix,aes(x = Time, ymin = areaMinProportion, ymax = areaMaxProportion),fill = 'grey50')+
    ggplot2::geom_line(data=fix,aes(x = Time, y= Value),color = 'black',alpha = 0.7,size =1)+
