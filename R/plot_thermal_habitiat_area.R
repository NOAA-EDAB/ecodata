...
plot_thermal_habitat_area <- function(
  shadedRegion = NULL,
  report = "MidAtlantic",
  EPU = "MAB"
) {
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::geom_line(
      data = fix,
      ggplot2::aes(x = temp.threshold, y = Value, group = Time, color = Time),
      alpha = 0.7,
      linewidth = 1.2
    ) +
    ...
    ggplot2::geom_line(
      data = dplyr::filter(fix.this.year, Time == max(Time)),
      ggplot2::aes(
        x = temp.threshold,
        y = Value,
        linetype = as.factor(ReportYear)
      ),
      color = 'black',
      alpha = 0.7,
      linewidth = 2
...
attr(plot_thermal_habitat_area, "EPU") <- c("MAB", "GB", "GOM")
attr(plot_thermal_habitat_area, "report") <- c("MidAtlantic", "NewEngland")
