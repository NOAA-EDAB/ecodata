...
plot_ch_bay_sal <- function(shadedRegion = NULL, report = "MidAtlantic") {
  ...
  p <- CBsal |>
    ggplot2::ggplot() +
    ggplot2::geom_ribbon(
      ggplot2::aes(x = Time, ymin = minLTA, ymax = maxLTA),
      fill = "grey",
      alpha = 0.5
    ) +
    ...
    ggplot2::geom_line(ggplot2::aes(
      x = Time,
      y = YearLTA,
      color = "Long Term Average 2010-2023"
    )) +
    ggplot2::geom_line(ggplot2::aes(x = Time, y = Year, color = "Daily 2024")) +
