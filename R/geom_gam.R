#'A geom to fit a GAN model to a time series
#'
...
#'   geom_gam(aes(x = x, y = y))

geom_gam <- function(mapping = NULL, data = NULL, stat = "GAM",
                     position = "identity", na.rm = FALSE, show.legend = NA,
                     inherit.aes = TRUE, warn = TRUE, ...) {
  ggplot2::layer(
    geom = ecodata:::GeomGAM, mapping = mapping, data = data, stat = stat,
