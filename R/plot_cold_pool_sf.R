...
plot_cold_pool_sf <- function(shadedRegion = NULL, report = "MidAtlantic") {
  ...
  area2 <- data.frame(Area = sf::st_area(area)) |>
    ...
    dplyr::mutate(Value = as.numeric(Value), hline = mean(Value))

  minCPyear <- as.numeric(area2$Time[which(
    area2$Value == min(area2$Value, na.rm = TRUE)
  )])
  maxCPyear <- as.numeric(area2$Time[which(
    area2$Value == max(area2$Value, na.rm = TRUE)
  )])
