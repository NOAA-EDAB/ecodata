...
plot_heatwave_year <- function(
  ...
  hwyr <- ecodata::heatwave_year |>
    dplyr::filter(EPU %in% filterEPUs, Year == year, Var == plotvar)
  ...
  if (nrow(hwyr) > 0) {
    p <- hwyr |>
      ...
      ggplot2::geom_line(
        ggplot2::aes(x = t, y = seas, color = "a"),
        linewidth = 1
      ) +
      ggplot2::geom_line(
        ggplot2::aes(x = t, y = thresh, color = "c"),
        linewidth = 1
      ) +
      ggplot2::geom_line(ggplot2::aes(x = t, y = temp, color = "b")) +
      ggplot2::scale_colour_manual(
        values = c("turquoise4", "sienna3", "black"),
