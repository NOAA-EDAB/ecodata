# Test that the plot function has working default arguments
test_that("'plot_wind_occupancy' has valid default arguments", {
  expect_no_error(invisible(ecodata::plot_wind_occupancy()))
})

# Test that the plot function creates an object
test_that("'plot_wind_occupancy' creates an object", {
  expect_type(invisible(ecodata::plot_wind_occupancy()), "object")
})

# Test that the plot function has user-defined arguments
test_that("'plot_wind_occupancy' has user-defined attributes", {
  expect_gte(length(attributes(ecodata::plot_wind_occupancy)), 1)
  expect_no_match(
    names(attributes(ecodata::plot_wind_occupancy)),
    "srcref",
    all = FALSE
  )
})

# Test that the plot function has valid argument combinations
test_that("'plot_wind_occupancy' all argument combinations work", {
  expect_no_error(ecodata::create_all_plots(
    ecodata_name = "wind_occupancy",
    print = FALSE
  ))
})
