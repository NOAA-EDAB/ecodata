...
test_that("'plot_comdat' has valid default arguments", {
  expect_no_error(invisible(ecodata::plot_comdat()))
})
...
test_that("'plot_comdat' has user-defined attributes", {
  expect_gte(length(attributes(ecodata::plot_comdat)), 1)
  expect_no_match(
    names(attributes(ecodata::plot_comdat)),
    "srcref",
    all = FALSE
  )
})
...
test_that("'plot_comdat' all argument combinations work", {
