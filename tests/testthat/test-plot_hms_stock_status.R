...
test_that("'plot_hms_stock_status' has valid default arguments", {
  expect_no_error(invisible(ecodata::plot_hms_stock_status()))
})

# Test that the plot function creates an object
test_that("'plot_hms_stock_status' creates an object", {
...
test_that("'plot_hms_stock_status' has user-defined attributes", {
  ...
  expect_no_match(
    names(attributes(ecodata::plot_hms_stock_status)),
    "srcref",
    all = FALSE
  )
})
...
test_that("'plot_hms_stock_status' all argument combinations work", {
  expect_no_error(ecodata::create_all_plots(
    ecodata_name = "hms_stock_status",
    print = FALSE
  ))
})
