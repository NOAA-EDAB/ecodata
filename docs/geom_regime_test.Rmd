...
#### Zooplankton Diversity Anomaly Bar
...
zoo_div %>% 
  ggplot2::ggplot(aes(x = Time, y = anom, group = Var)) +
  ggplot2::annotate("rect", fill = shade.fill, alpha = shade.alpha,
      xmin = x.shade.min , xmax = x.shade.max,
      ymin = -Inf, ymax = Inf) +
  ecodata::geom_regime()+
...
#### Zooplankton Diversity Standardized Anomaly Bar

```{r}
zoo_div <- ecodata::zoo_diversity %>% 
  dplyr::filter(EPU == "MAB") %>% 
  dplyr::mutate(anom = Value/mean(Value)-1, 
                anom_sd = anom/sd(Value))

zoo_div %>% 
  ggplot2::ggplot(aes(x = Time, y = anom_sd, group = Var)) +
  ggplot2::annotate("rect", fill = shade.fill, alpha = shade.alpha,
      xmin = x.shade.min , xmax = x.shade.max,
      ymin = -Inf, ymax = Inf) +
  ecodata::geom_regime()+
  ggplot2::geom_col(aes(fill = ifelse(anom_sd<0, "red", "blue")))+
  #ggplot2::geom_point() +
  ggplot2::ylab("Standardized Anomaly") +
  ggplot2::xlab(element_blank())+
  ggplot2::ggtitle("Zooplankton Diversity") +
  ggplot2::facet_wrap(Var~., ncol = 3) +
  ggplot2::scale_x_continuous(expand = c(0.01, 0.01))+
...
exp2<- ecodata::exp_n  %>% 
  ...
  dplyr::mutate(upper = Value+ValueSD, 
         lower = Value - ValueSD, 
         anom = Value/mean(Value) - 1)



exp2 %>% 
ggplot2::ggplot(aes(x = Time, y = anom)) +
  ecodata::geom_regime(data = exp2, aes(x = Time, y = anom, fill = NULL))+
  #Highlight last ten years
  ggplot2::annotate("rect", fill = shade.fill, alpha = shade.alpha,
      xmin = x.shade.min , xmax = x.shade.max ,
      ymin = -Inf, ymax = Inf) +
