...
ecodata::harborporpoise %>% 
  ...
  rename(
    #Year   = Time, 
    # EST    = `harbor porpoise bycatch estimate`, 
    # PBR    = `harbor porpoise bycatch pbr`,
    # lo95ci = `harbor porpoise bycatch lo95ci`,
    # up95ci = `harbor porpoise bycatch up95ci`) %>% 
    Year   = YEAR, 
    lo95ci = LCI,
    up95ci = UCI) %>% 
  ggplot2::ggplot()+
...
ecodata::grayseal %>% 
  ...
  mutate(
    EST = unlist(EST),
    PBR = unlist(PBR),
    lo95ci = unlist(lo95ci),
    up95ci = unlist(up95ci)) %>% 
  ggplot2::ggplot()+
  ggplot2::geom_point(aes(x=Year, y = EST), color = "black")+
  ggplot2::geom_line(aes(x=Year, y = EST), color = "black")+
  ggplot2::geom_point(aes(x=Year, y = PBR), color = "red")+
