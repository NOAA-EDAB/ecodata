...
### Spring Stock Area - North 
```{r bsb-spring-north, fig.cap="Seasonal sea surface temperature time series overlaid onto 2020 seasonal spatial anomalies for black seabass spring stock area."}
annotation_custom2 <- function (grob, xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, data) 
{
  layer(data = data, stat = StatIdentity, position = PositionIdentity, 
        geom = ggplot2:::GeomCustomAnn,
...
#EPU shapefile
poly <- ecodata::ESP_sf %>% 
  dplyr::filter(ID %in% c("black_sea_bass_north_spring")) 
...
winter_anom <-  ggplot2::ggplotGrob( ecodata::ESP_seasonal_oisst_anom %>% 
                              dplyr::filter(ESP == "black_sea_bass_north_spring",
                                     stringr::str_detect(Var, "winter")) %>% 
                              dplyr::mutate(hline = mean(Value)) %>% 
                              ggplot2::ggplot(aes(x = Time, y = Value)) +
                              ggplot2::annotate("rect", fill = shade.fill, alpha = shade.alpha,
...
### Spring Stock Area - South
```{r bsb-spring-south, fig.cap="Seasonal sea surface temperature time series overlaid onto 2020 seasonal spatial anomalies for black seabass spring stock area."}
annotation_custom2 <- function (grob, xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, data) 
{
  layer(data = data, stat = StatIdentity, position = PositionIdentity, 
        geom = ggplot2:::GeomCustomAnn,
...
#EPU shapefile
poly <- ecodata::ESP_sf %>% 
  dplyr::filter(ID %in% c("black_sea_bass_south_spring")) 
...
winter_anom <-  ggplot2::ggplotGrob( ecodata::ESP_seasonal_oisst_anom %>% 
                              dplyr::filter(ESP == "black_sea_bass_south_spring",
                                     stringr::str_detect(Var, "winter")) %>% 
                              dplyr::mutate(hline = mean(Value)) %>% 
                              ggplot2::ggplot(aes(x = Time, y = Value)) +
                              ggplot2::annotate("rect", fill = shade.fill, alpha = shade.alpha,
