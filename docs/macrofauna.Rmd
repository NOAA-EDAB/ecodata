...
load(file= file.path(raw.dir,"Survdat_proportions.Rdata"))

mafmc_ne <- survey_proportions %>% 
  distinct() %>% 
  filter(EPU %in% c("GOM","GB"),
         Fed_Managed == "MAFMC",
         !str_detect(group,"Other")) %>% 
  group_by(EPU, group, season, Time) %>% 
  dplyr::summarise(Value = sum(Prop)) %>% 
  unite(.,Var,c("group","season"), sep = " ") %>% 
  group_by(EPU,Var) %>% 
  mutate(hline = mean(Value)) %>% 
...
bar_dat <- ecodata::recruitment_anomaly %>% 
  filter(EPU == "MAB")
...
```{r hp-density, fig.width = 8, fig.height = 3.8}
fall_hp <- 
   hp_density %>% 
  filter(Season == "Fall") %>% 
  ggplot() +
...
spring_hp +fall_hp +  plot_layout(ncol = 2) + plot_annotation(title = "Harbor porpoise density")
```
