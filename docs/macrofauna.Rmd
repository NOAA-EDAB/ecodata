...
```{r agg-bio, fig.cap = paste0("Spring (left) and fall (right) surveyed biomass in the Mid-Atlantic Bight. Data from the NEFSC Bottom Trawl Survey are shown in black, with NEAMAP shown in red. \\label{nefsc-biomass}"), fig.width=8, fig.asp = 0.75}
agg<-ecodata::agg_bio %>% 
...
agg1<-agg %>% filter(stat == "Mean") %>% 
  group_by(Time, EPU, Var) %>% 
  spread(stat, Value)
agg2<-agg %>% filter(stat == "SD", 
                     !str_detect(Var, "NA")) %>% 
  group_by(Time, EPU, Var) %>% 
  spread(stat, Value)

agg3<-agg1 %>% cbind(agg2) %>% 
  dplyr::select(Time, Var, Units, Mean, SD) %>% 
  group_by(Time, Var, EPU) %>% 
  mutate(upper = Mean + SD, 
         lower = Mean - SD)


agg_bio<-agg3 %>% filter(EPU == epu_abbr,
         Time >= 1968) %>% 
  group_by(Var, EPU) %>% 
...
p1<-agg_bio %>% 
  ...
  geom_line(aes(x = Time, y = upper))+
  geom_line(aes(x = Time, y = lower))+
...
agg_bio<-agg3 %>% filter(EPU %in% c("GOM","GB"),
         Time >= 1968) %>% 
  group_by(Var, EPU) %>% 
  mutate(hline = mean(Mean, na.rm = T)) %>% 
  ungroup() 
