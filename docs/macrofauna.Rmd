...
```{r tern-diet-diversity, fig.cap = "Evenness and Shannon diversity of common tern diets observed at seven different islands in Gulf of Maine. Diversity of common tern diets has been predominantly above the long-term mean since 2006."}
...
diet_div <- ecodata::common_tern %>% 
  ...
  dplyr::summarize(Value = mean(Value, na.rm = T),
                   sd = sd(Value, na.rm = T),
                   n = n()) %>%
  mutate(Value = ifelse(n <= 3, NA, Value)) %>% 
  group_by(Var) %>% 
  mutate(hline = mean(Value, na.rm = T))
...
evenness + shannon + plot_layout(ncol = 1)
```
...
diet_freq_bar <- prey_freq %>% 
  ...
  geom_bar_interactive(aes(tooltip = paste(Prey,round(Freq,2))),stat = "identity") +
