...
total_surv$Var <- factor(total_surv$Var,levels = c("Piscivore Spring",
                                                   "Piscivore Fall",
                                                    "Benthivore Spring",
                                                   "Benthivore Fall",
                                                    "Planktivore Spring",
...
neamap$Var <- factor(neamap$Var,levels = c("Piscivore Spring","Piscivore Fall",
                                           ...
                                           "Planktivore Spring", "Planktivore Fall",                                           "Benthos Spring", "Benthos Fall"))
...
p1<-total_surv %>% 
  ...
  #Add NEAMAP
  geom_line(data = neamap.1, aes(x = Time, y = Value, color = Var),
            size = lwd-0.5,
            color = "#ca0020")+
  geom_point(data = neamap.1, aes(x = Time, y = Value, color = Var),
             size = pcex-0.5,
             color = "#ca0020")+
...
p2<-total_surv %>% 
  ...
  #Add NEAMAP
  geom_line(data = neamap.2, aes(x = Time, y = Value, color = Var),
            size = lwd-0.5,
            color = "#ca0020")+
  geom_point(data = neamap.2, aes(x = Time, y = Value, color = Var),
             size = pcex-0.5,
             color = "#ca0020")+
...
p3<-total_surv %>% 
  ...
  #Add NEAMAP
  geom_line(data = neamap.3, aes(x = Time, y = Value, color = Var),
            size = lwd-0.5,
            color = "#ca0020")+
  geom_point(data = neamap.3, aes(x = Time, y = Value, color = Var),
             size = pcex-0.5,
             color = "#ca0020")+
...
p4<-total_surv %>% 
  ...
  #Add NEAMAP
  geom_line(data = neamap.4, aes(x = Time, y = Value, color = Var),
            size = lwd-0.5,
            color = "#ca0020")+
  geom_point(data = neamap.4, aes(x = Time, y = Value, color = Var),
             size = pcex-0.5,
             color = "#ca0020")+
...
```{r MAB-recruitment, fig.cap = "Small fish per large fish biomass anomaly in the Mid-Atlantic Bight.",fig.width = 8, fig.height = 5}
...
[Maybe best as narrative] (https://ccbbirds.org/wp-content/uploads/CCBTR-19-06_Colonial-waterbirds-in-coastal-Virginia-2018.pdf) - well documented decline in birds in last 20 years.  Since the 1980s, Common Terns have shown consistent declines on the barrier islands (Williams et al. unpublished data).. Between 1993 and 2018, Common Terns declined by 80.6% in coastal Virginia. Considerable declines have been documented in all 3 geographic regions that supported colonies in 1993.
```{r VA-cote, fig.cap = "Number of breeding common tern pairs on VA barrier islands. Error bars show +/- 1 SE of the mean.",fig.width = 5, fig.asp = 0.55 }
...
total_surv <- nefsc_survey %>% 
  ...
  mutate(Var = word(Var, 1,2))
series.col <- rep("black",length(unique(total_surv$Var)))
total_surv$Var <- factor(total_surv$Var,levels = c("Piscivore Spring","Piscivore Fall",
                                                  "Benthivore Spring","Benthivore Fall",
                                                  "Planktivore Spring", "Planktivore Fall",
...
```{r NEFMC-in-MAB, fig.width = 7, fig.asp = 0.45}
...
mass <- mass_inshore_survey %>% 
  filter(EPU == "GB",
         Time>1980,
         str_detect(Var, "Index")) %>% 
  mutate(feeding.guild = word(Var, 1)) %>%
...
### Productivity anomaly
```{r NE-recruitment, fig.cap = "Small fish per large fish biomass anomaly in New England.",fig.width = 8, fig.height = 5}
...
#### GB
```{r gb-cf, fig.cap = "Condition factor for species sampled in GB."}
image.dir <- here::here("docs")

knitr::include_graphics(file.path(image.dir, "GBcondition_2019.jpg"))

```

#### GOM
```{r gom-cf, fig.cap = "Condition factor for species sampled in the GOM."}
image.dir <- here::here("docs")

knitr::include_graphics(file.path(image.dir, "GOMcondition_2019.jpg"))
...
```{r stacked-bar-prey-freq, fig.cap = "Prey frequencies in the diets of common tern observed at seven different islands in Gulf of Maine.", fig.height = 4, fig.width = 8}
...
```{r NARW-abundance, fig.width = 6, fig.asp = 0.55,fig.cap = "Estimated North Atlanic right whale abundance. Upper and Lower 95 in red."}
...
```{r NARW-calf-abundance, fig.width = 6, fig.asp = 0.55,fig.cap = "Number of North Atlantic right whale calf births, 1990 - 2019."}
hline <- ecodata::narw %>% 
  filter(Var == "Calves") %>% 
  drop_na() %>% 
...
```{r blue-runner, fig.width = 6, fig.asp = 0.55,fig.cap = "Blue runner presence on Northeast Shelf"}
