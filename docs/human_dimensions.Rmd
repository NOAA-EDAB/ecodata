...
ggplot(data = stock_status) +
  ...
  geom_text_repel(aes(x = B.Bmsy, #geom_text_repel auto-jitters text around points
                      y = F.Fmsy,
                      label = Code), show.legend = FALSE,nudge_y = 0.05, nudge_x = 0.05) +
...
NC<-d %>% 
  ...
  ggtitle("Commercial Engagement")+
  theme_ts()
cowplot::plot_grid(NY, NJ, MD, VA, NC, ncol = 1) 
```
...
ggplot(data = stock_status) +
  ...
  annotate("rect", xmin = 0.924*x.max,
           ...
           ymax = 0.98*y.max,
           alpha = 0.01) +
  annotate("text", x = 6.2, y = 2.5, label = "F and B missing", fontface =2, size = 3)+
  annotate("rect",  
             xmin = 0.70*x.max,
             xmax = 0.85*x.max,
             ymin = 0.90*y.max,
             ymax = 0.98*y.max,
             alpha = 0.01) +
  annotate("text", x = 4.8, y = 2.5, label = "B missing", fontface =2, size = 3)+
  annotate("rect", xmin = 0.509*x.max,
           xmax = 0.681*x.max,
           ymin = 0.65*y.max,
           ymax = 0.98*y.max,
           alpha = 0.01) +
  annotate("text", x = 3.2, y = 2.5, label = "F missing", fontface =2, size = 3)+
  xlab(expression(~B/B[msy])) +
  ylab(expression(~F/F[msy])) +
  guides(color = FALSE) +
  theme_ts()
...
cowplot::plot_grid(ME, NH, MA, RI, CT, ncol = 2) 
```
...
```{r wind_habitat_table, eval = T, echo = F}
raw.dir <- here::here("data-raw")
wind.occur<-read.csv(file.path(raw.dir, "wind_habitat.csv")) %>% 
  rename( "Species" = "Species.1", 
          ...
          "Spring" = "spring", 
          "Spring" = "spring.1")
kable(wind.occur, caption = "Average occurence probability of top ten species across existing and proposed wind lease areas in both spring and fall.") %>% 
  add_header_above(c("Existing" = 4, "Proposed" = 4)) %>% 
...
#planned_shp <- readOGR(file.path(gis.dir, "BOEM_MHK_Planning_Areas_2_13_2019.shp"), verbose = F)## Currently west coast???
...
```{r hms_rev, eval = T, echo = F, fig.cap="Revenue from 19 highly migratory species landings (Sharks, Tunas, Swordfish)" }
hms_landings_rev %>%  mutate(Time = as.character(Time)) %>% 
  group_by(Time, Region) %>% 
  summarise(tot = sum(Value)/1000000) %>% 
  group_by(Time) %>% 
  mutate(perc = as.integer(tot/(sum(tot))*100)) %>% 
  ungroup() %>% 
  ggplot() +
  aes(x = Time, y = tot, fill = Region, label = paste0(perc, '%')) +
  geom_bar(stat = "identity", width = 0.5) +
  geom_text(size = 4, position =position_stack(vjust = 0.5)) +
  ggtitle("Revenue from 19 highly migratory species (Sharks, Tunas, Swordfish)")+
  ylab(expression("Revenue (10"^6*"USD)"))+
  theme_ts()

```

```{r hms_weight, eval = T, echo = F, fig.cap="Landings for 19 Highly migratory species (Sharks, Tunas, Swordfish)." }
hms_landings_weight %>%  mutate(Time = as.character(Time)) %>% 
  group_by(Time, Region) %>% 
  summarise(tot = (sum(Value)/2205)/1000) %>% 
  group_by(Time) %>% 
  mutate(perc = as.integer(tot/(sum(tot))*100)) %>% 
  ungroup() %>% 
  ggplot() +
  ...
  geom_bar(stat = "identity", width = 0.5) +
  geom_text(size = 4, position =position_stack(vjust = 0.5)) +
  ggtitle("Highly Migratory Species Landings")+
  ylab(expression("Landings (10"^3*"metric tons)"))+
  theme_ts()
