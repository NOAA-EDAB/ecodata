...
```{r ppr-mab, fig.cap="Primary production required to support lanndings."}
ecodata::ppr %>% 
  group_by(EPU) %>% 
  ...
  ylab("% Total Primary Production Required")+
  theme_ts()
```
...
### Primary Production Required
```{r ppr-ne, fig.cap="Primary production required to support lanndings."}
ecodata::ppr %>% 
  group_by(EPU) %>% 
  mutate(Value = Value*100, 
         hline = mean(Value)) %>% 
