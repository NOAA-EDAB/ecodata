...
ecodata::zoo_abund%>%
   ...
   xlab(element_blank())+
   ggtitle("Small and large copepod abundance anomaly") +
   scale_x_continuous(expand = c(0.01, 0.01))+
       geom_hline(aes(yintercept = hline),
            size = hline.size,
            alpha = hline.alpha,
...
```{r NE-zoo-abund, fig.cap= "Large (red) and small (blue) copepod zooplankton abundance in New England."}
ecodata::zoo_abund %>%
  filter(EPU %in% c("GOM","GB")) %>%
  mutate(hline = 0) %>% 
  ggplot(aes(x = Time, y = Value, color = Var)) +
...
knitr::include_graphics(file.path(image.dir,"GOM_Heatwave_timeseries.jpg"))
```
