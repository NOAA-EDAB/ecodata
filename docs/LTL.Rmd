...
interp_chl <- function(epu, int_step, year = 2018){
  out <- ecodata::chl_pp %>% 
          filter(str_detect(Var,"WEEKLY_CHLOR_A_MEDIAN MODIS-Aqua PAN"),
                 EPU == epu) %>% 
...
MAB_chl <- interp_chl(epu = "MAB", int_step = 1000)
...
GB_chl_weekly <- interp_chl(epu = "GB", int_step = 1000, year = 2018)
...
GOM_chl_weekly <- interp_chl(epu = "GOM", int_step = 1000, year = 2018)
...
```{r NAO, fig.width = 8, fig.asp = .35}
nao_ann <- ecodata::nao %>% 
  group_by(Year) %>% 
  dplyr::summarise(Mean = mean(Value),
                   Variance = var(Value, na.rm = T)) %>% 
  mutate(hline = 0,
         hline.var = mean(Variance)) %>% 
