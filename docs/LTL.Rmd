...
sst_map + 
  annotation_custom2(grob = winter_anom,  xmin=-72.5, xmax=-64.25,
                     ymin=35.25, ymax=39.25, data = data.frame(Season = "Winter")) +
  annotation_custom2(grob = spring_anom,  xmin=-72.5, xmax=-64.25,
                     ymin=35.25, ymax=39.25, data = data.frame(Season = "Spring")) +
  annotation_custom2(grob = summer_anom,  xmin=-72.5, xmax=-64.25,
                     ymin=35.25, ymax=39.25, data = data.frame(Season = "Summer")) +
  annotation_custom2(grob = fall_anom,  xmin=-72.5, xmax=-64.25,
                     ymin=35.25, ymax=39.25, data = data.frame(Season = "Fall"))
...
```{r chl-monthly, fig.width = 8, eval = F}
...
```{r mab-chl-weekly,fig.width = 6, fig.asp = 0.55, fig.cap = "Weekly chlorophyll concentrations in the Mid-Atlantic are shown for by the colored line for 2018. The long-term mean is shown in black and shading indicates +/- 1 sample SD."}
...
MAB_chl_weekly <- ggplot(data = MAB_chl) +
  ...
  geom_line(aes(x = Time, y = Value, color = status, group = group),
            size = 1) +
  ggtitle(expression("MAB chlorophyll"~italic(a)~"")) +
  guides(color = F) +
  xlab("")+
  ylab(expression("CHL (mg m"^-3*")")) +
...
# MAB_pp <- interp_chl_pp(epu = "MAB", int_step = 1000,Variable =  "WEEKLY_PPD_MEDIAN OC-CCI BERENFELD")
# 
...
#   geom_line(aes(x = Time, y = Value, color = status, group = group),
#             size = 1) +
#   ggtitle(expression("MAB primary production")) +
#   guides(color = F) +
#   xlab("")+
#   ylab(expression("PP (gC m"^-2*" d"^-1*")")) +
#   scale_x_continuous(breaks = seq(0,50,10),
...
pp_anom <- ecodata::chl_pp %>% 
  filter(str_detect(Var, "ANNUAL_PPD_RATIO_ANOMALY OC-CCI BERENFELD 1998_2017"),
         EPU == "MAB") %>% 
  mutate(hline = mean(Value)) %>% 
  rbind(.,sli)

pp_anom %>% 
  ggplot(aes(x = Time, y = Value, group = Var, color = Var)) +
         annotate("rect", fill = shade.fill, alpha = shade.alpha,
      xmin = x.shade.min , xmax = x.shade.max,
      ymin = -Inf, ymax = Inf) +
  geom_line() +
...
```{r NE-SST-insitu, fig.width=8, fig.height = 7}
annotation_custom2 <- function (grob, xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, data) 
{
  layer(data = data, stat = StatIdentity, position = PositionIdentity, 
        geom = ggplot2:::GeomCustomAnn,
...
sst_map + 
  annotation_custom2(grob = winter_anom,  xmin=-72, xmax=-64, ymin=35.25, ymax=39.25, data = data.frame(Season = "Winter")) +
  annotation_custom2(grob = spring_anom,  xmin=-72, xmax=-64, ymin=35.25, ymax=39.25, data = data.frame(Season = "Spring")) +
  annotation_custom2(grob = summer_anom,  xmin=-72, xmax=-64, ymin=35.25, ymax=39.25, data = data.frame(Season = "Summer")) +
  annotation_custom2(grob = fall_anom,  xmin=-72, xmax=-64, ymin=35.25, ymax=39.25, data = data.frame(Season = "Fall"))
...
```{r NE-chl-weekly,fig.width = 6, fig.asp = 1.1, fig.cap = "Weekly chlorophyll concentrations for 2018 in Gulf of Maine and Georges Bank are shown by the colored lines in the above figures. The long-term mean is shown in black and shading indicates +/- 1 sample SD."}

GB_chl_weekly <- interp_chl_pp(epu = "GB", int_step = 1000, year = 2018,Variable = "WEEKLY_CHLOR_A_MEDIAN MODIS-Aqua PAN")
...
GOM_chl_weekly <- interp_chl_pp(epu = "GOM", int_step = 1000, year = 2018,Variable = "WEEKLY_CHLOR_A_MEDIAN MODIS-Aqua PAN")
...
GOM_chl + GB_chl + plot_layout(ncol = 1) 
