...
```{r hms-landings-comdat-commercial-landings, warning=F, fig.length = 24, fig.cap = "NEFMC managed species landings (red) and total commercial landings (black) by feeding guild in Gulf of Maine (left) and Georges Bank (right)."}
...
```{r commercial-engagement, message=F, fig.cap= "Commercial engagement and reliance for the top top commercial fishing communities in New England."}
...
com2<-com %>% 
  ...
  ggrepel::geom_label_repel(aes(x = Eng, #geom_text_repel auto-jitters text around points
                      y = Rel,
                      label = Community,
...
```{r recreational-engagement, message=F, fig.cap= "Commercial engagement and reliance for the top top recreational fishing communities in New England."}
...
com2<-com %>% 
  ...
  ggplot2::ggtitle("Social Vulnerability in Top Recreation Fishing Communities")
  ggplot2::guides(color = FALSE) +
  ecodata::theme_ts()
...
ecodata::wind_revenue %>% 
 ...
 # dplyr::mutate(hline = mean(Value)) %>% 
  ...
  #         linetype = hline.lty)+
  ggplot2::scale_x_continuous(expand = c(0.01, 0.01)) +
  ggplot2::ggtitle("Wind Energy Revenue")+
