...
temp <- ecodata::seasonal_oisst_anom %>%
  ...
  dplyr::select(!Units) %>% 
  dplyr::mutate(Var = recode(Var, "Fall OISST anomaly" ="Temperature"))
...
forage<- ecodata::forage_index %>% 
  ...
  dplyr::mutate(Var = recode(Var, "Fall Forage Fish Biomass Estimate" ="Forage"))
...
condition<- ecodata::condition %>% 
  dplyr::group_by(Time) %>% 
  dplyr::summarise(anom = Value-(mean(Value))) %>% 
  dplyr::mutate(Var = "Condition", 
                EPU = "GOM")

gom <- rbind(temp, zoo, forage, condition) 
