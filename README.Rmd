...
[![Run PR Checks](https://github.com/NOAA-EDAB/ecodata/actions/workflows/verify_pr.yml/badge.svg)](https://github.com/NOAA-EDAB/ecodata/actions/workflows/verify_pr.yml)
<!-- badges: end -->

## Overview

[`ecodata`](https://noaa-edab.github.io/ecodata/) is an R data package developed by the Ecosystems Dynamics and Assessment Branch of the Northeast Fisheries Science Center
...
Management Councils (FMCs), and therefore indicator data for Scotian Shelf are included when available, but this is not always the case. `ecodata` has two companion bookdown documents: 1) [Tech-Doc](https://noaa-edab.github.io/tech-doc/), which details the methods used in the SOE, and 2) the [Northeast US Ecosystem Indicator Catalog](https://noaa-edab.github.io/catalog/), which provides expanded ecological context for SOE indicators. The `ecodata` package can be used to access to all of the indicators, datasets and plots featured in the SOE reports, which can be accessed using the table below. **Please consult the [technical documentation](https://noaa-edab.github.io/tech-doc/) of SOE indicators before using data sets.**
...
For data used in the past, please replace "X.X.X" below with the desired [`ecodata` version](https://github.com/NOAA-EDAB/ecodata/releases):
