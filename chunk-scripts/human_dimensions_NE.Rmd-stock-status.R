...
all.df <- data.frame(text = all_missing$Code,
                    x = rep(x.max*0.95,length(all_missing$Code)),
                    y = seq(1.45,0.7, length.out = 11))
b.df <- data.frame(text = b_missing$Code,
                    x = rep(x.max*0.55,length(b_missing$Code)),
                    y = seq(1.45,1.38, length.out = 2))
f.df <- data.frame(text = f_missing$Code,
                    x = rep(x.max*0.75,length(f_missing$Code)),
...
ggplot2::ggplot(data = stock_status) +
  ...
  ggplot2::annotate("rect", xmin = 0.924*x.max,
           ...
           ymax = 0.98*y.max,
           alpha = 0.01) +
  ggplot2::annotate("text", x = 4.7, y = 1.5, label = "F and B missing", fontface =2, size = 3)+
  ggplot2::annotate("rect",  
             ...
             ymax = 0.98*y.max,
             alpha = 0.01) +
  ggplot2::annotate("text", x = 2.7, y = 1.5, label = "B missing", fontface =2, size = 3)+
  ggplot2::annotate("rect", xmin = 0.509*x.max,
           xmax = 0.681*x.max,
           ymin = 0.65*y.max,
           ymax = 0.98*y.max,
           alpha = 0.01) +
  ggplot2::annotate("text", x = 3.7, y = 1.5, label = "F missing", fontface =2, size = 3)+
